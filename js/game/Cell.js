...
export class Cell {
    ...
    constructor(core, id, x, y, r, sprite, name, color) {
        ...
        this.sprite.sortableChildren = false; // включается при нике/эффектах
    ...
    _updateFoodLod(force = false) {
        ...
        // На desktop/Mac при большом кол-ве клеток раньше уходим в simple LOD.
  ...
  /** Позиция ряда квадратиков под name (локальные координаты, R=256). */
    ...
    _getNameTexture(name) {
        ...
        // Генерация текстуры (2× достаточно; 3× на Retina сильно грузит GPU)
        const texture = this.core.app.renderer.generateTexture(text, {
            resolution: Math.min(2, this.core.app.renderDpr || 2),
